...
Oltre al livello di prezzo, l'unica dimensione catturata dai modelli convenzionali, esiste un asse di persistenza temporale che determina quanto a lungo uno shock si propaga nel sistema: conoscere il regime di prezzo non dice quasi nulla sulla velocità di mean-reversion, e viceversa.
...
Tra i foundation model disponibili al momento dello studio, adottiamo MOMENT \citep{moment2024} perché è particolarmente adatto: è progettato specificamente per serie temporali (non adattato da modelli linguistici), fornisce una modalità nativa di embedding per l'apprendimento delle rappresentazioni (non solo per la previsione), ed è interamente open source, a differenza di alternative come Chronos, TimesFM e Moirai che non soddisfano contemporaneamente tutti e tre i requisiti.
...
Un valore di $\alpha$ vicino a zero indica che il processo è quasi un random walk e lo shock persiste indefinitamente, mentre un valore vicino a uno indica una rapida mean-reversion.
La half-life, cioè il tempo necessario per dimezzare uno shock, è $t_{1/2} = -\ln 2 / \ln(1-\alpha)$: con $\alpha = 0{,}15$ la half-life è di circa 4 ore.
Il parametro $\alpha$ è direttamente misurabile dall'autocorrelazione al lag~1 del residuo all'interno di ogni finestra di 512 ore, dato che $\text{ACF}(1) \approx 1 - \alpha$, e ogni finestra~$i$ ha quindi il proprio $\hat{\alpha}_i = 1 - \widehat{\text{ACF}}_1(r_i)$ calcolato sui livelli $r_t$.
La volatilità è invece misurata sugli incrementi stazionari, $\hat{\sigma}_i = \text{std}(\Delta r_i)$: la mean-reversion $\alpha$ è stimata sulla variabile che MOMENT riceve e sulla quale i regimi $D$ sono definiti, la volatilità $\sigma$ sulla variabile su cui FE opera, e l'unità di osservazione è la finestra ($N = 7\,217$), non la singola ora.
...
\end{equation}
Confrontiamo due parametrizzazioni che differiscono nella granularità con cui $\alpha$ è stimato: il modello a un asse, $\alpha(E)$ e $\sigma(E)$, in cui tutti i parametri sono mediati per regime economico $E$, e il modello a due assi, $\alpha(E,D)$ e $\sigma(E,D)$, in cui i parametri sono mediati per cella dello spazio congiunto.
...
Nel sotto-regime a rapida reversione ($\alpha \approx 0{,}16$, half-life $\approx 4$\,h), il modello a un asse applica $\alpha = 0{,}078$ (half-life 8,6\,h), sovrastimando la persistenza quasi del doppio; le coperture vengono calibrate su uno shock che nella realtà rientra in mezza giornata, con il risultato di immobilizzare capitale inutilmente.
Nel sotto-regime persistente ($\alpha \approx 0{,}03$, half-life $\approx 22$\,h), accade l'opposto: il modello a un asse sottostima la persistenza di un fattore 2,5$\times$, e le posizioni di copertura risultano insufficienti proprio nei periodi in cui gli shock durano più di un giorno.
...
Il modello convenzionale a singola etichetta di regime non è in grado di rappresentare queste combinazioni, e l'asse dinamico fornisce la dimensione mancante per una caratterizzazione del rischio che va oltre il livello nominale di prezzo.
...
Dinamico (MOMENT) & $K_D = 9$ & 0,420$^\text{ACF}$  & ---  & ---  & 36/36 \\
\bottomrule
\end{tabular}
...
Come discusso nella Sezione~\ref{sec:repr_fe}, la separazione tra i due assi è una conseguenza della matematica degli input: FE opera sugli incrementi stazionari e cattura la distribuzione degli shock, MOMENT opera sui livelli persistenti e cattura la dinamica temporale.
Il meccanismo specifico che rende MOMENT efficace è il suo obiettivo di pre-addestramento basato sulla ricostruzione mascherata (\emph{masked time-series modeling}), che forza l'encoder del transformer ad analizzare i pattern nel loro contesto temporale, rendendolo architetturalmente idoneo a codificare le variazioni della memoria stocastica.
Il risultato è un asse di persistenza che governa il parametro di mean-reversion $\alpha$ ($\Delta\text{BIC} = -3\,875$, Sezione~\ref{sec:model_insufficiency}) e che nessuna combinazione di feature distribuzionali è in grado di isolare.

Un'obiezione naturale è se il foundation model sia davvero necessario, o se un clustering diretto sulle autocorrelazioni campionarie produca la stessa partizione.
Per verificarlo, abbiamo costruito una baseline che utilizza come rappresentazione le sole ACF ai lag 1, 6, 24 e 168 ore, calcolate sui livelli $r_t$, e la processa con la stessa pipeline (Diffusion Maps, ToMATo, Tukey HSD).
La partizione risultante separa $\alpha$ in modo più netto di MOMENT ($\eta^2 = 0{,}88$ contro $0{,}38$), ma è sostanzialmente diversa dalla partizione MOMENT (ARI $= 0{,}07$) e produce un numero maggiore di regimi (14 contro 9).
La superiorità della baseline in termini di $\eta^2$ è attesa: le ACF \emph{sono} la persistenza, calcolata direttamente, mentre MOMENT la codifica indirettamente attraverso un embedding ad alta dimensionalità.
Il valore di MOMENT non sta nella potenza di separazione, ma nella scoperta: il foundation model identifica autonomamente che la persistenza temporale è la dimensione organizzativa dominante, senza che sia necessario specificare a priori quali feature calcolare.
Questo rende il risultato indipendente dalle scelte di dominio e ne rafforza la generalizzabilità.

Una seconda obiezione è se la quasi-ortogonalità sia un artefatto della scelta degli input, dato che FE opera su $\Delta r_t$ (senza memoria) e MOMENT su $r_t$ (senza livello di prezzo).
Tre controfattuali empirici mostrano che non è così.
Primo, arricchendo FE con le 4 ACF calcolate sui livelli $r_t$, la concordanza con MOMENT sale solo da ARI $= 0{,}012$ a $0{,}019$.
Secondo, applicando MOMENT alla serie arcsinh non destagionalizzata (dove il livello di prezzo è ancora presente), la concordanza con FE resta ARI $= 0{,}001$: MOMENT non cattura il prezzo nemmeno quando potrebbe.
Terzo, un clustering sulle sole ACF è quasi ortogonale a entrambi gli assi (ARI $= 0{,}024$ con $E$, $0{,}039$ con $D$).
La separazione tra i due assi sopravvive a variazioni della rappresentazione e non dipende dalla specifica scelta degli input.
...
Tuttavia, gli $\eta^2$ calcolati sul sottocampione di finestre non sovrapposte sono sostanzialmente invariati ($\eta^2(D) = 0{,}371$ contro $0{,}376$), confermando che la struttura dei regimi non è un artefatto della pseudo-replicazione.
