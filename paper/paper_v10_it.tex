...
Il mercato elettrico è organizzato lungo due assi indipendenti, non uno.
...
Questo lavoro scopre questa struttura bidimensionale attraverso una pipeline interamente non supervisionata e ne quantifica le conseguenze per la modellazione e la gestione del rischio.
...
La letteratura ha ampiamente documentato che entrambe le dimensioni presentano struttura: il volatility clustering, uno degli stylized fact più robusti dei mercati finanziari \citep{cont2001}, è presente anche nei prezzi elettrici, e gli spike tendono ad aggregarsi in cluster persistenti piuttosto che presentarsi isolati \citep{christensen2009}.
Tuttavia, nessun approccio si chiede se queste due dimensioni siano governate dallo stesso processo latente o da processi indipendenti: la domanda viene semplicemente elusa, e un'unica etichetta di stato collassa livello e dinamica in una sola variabile.
...
Che la dinamica intra-giornaliera dei prezzi elettrici sia eterogenea è stato documentato empiricamente \citep{karakatsani2008, knittel2005}: la velocità con cui il mercato assorbe uno shock varia nel tempo e tra condizioni operative diverse.
Tuttavia, i modelli regime-switching esistenti \citep{huisman2003} assegnano un singolo stato latente che governa simultaneamente sia il livello di prezzo sia la dinamica della volatilità, assumendo implicitamente che le due dimensioni siano guidate dallo stesso processo.
Se questa assunzione sia valida, se cioè livello e dinamica siano effettivamente accoppiati o varino indipendentemente, è una domanda empirica che non è stata testata.
In sintesi, la letteratura ha accumulato evidenza che sia la volatilità sia la persistenza dei prezzi elettrici presentano una struttura propria, ma non ha mai verificato se questa struttura sia indipendente dal livello di prezzo.
Questo lavoro colma esattamente questo gap.

% --- 1.4 Nuovi strumenti ---

Due sviluppi recenti creano l'opportunità di testare questa ipotesi, e, aspetto cruciale, di farlo in modo interamente non supervisionato e guidato dai dati, eliminando ogni possibilità che scelte soggettive predeterminino il risultato.
...
Tra i foundation model disponibili, adottiamo MOMENT \citep{moment2024} perché soddisfa tre requisiti simultaneamente: è progettato specificamente per serie temporali (non adattato da modelli linguistici), fornisce una modalità nativa di embedding pensata per l'apprendimento delle rappresentazioni (non solo per la previsione), ed è interamente open source, con pesi, codice e dati di addestramento pubblicamente disponibili, garantendo la piena riproducibilità dei risultati.
...
È importante sottolineare che le due rappresentazioni non sono costruite su dati diversi, ma derivano entrambe dalla stessa serie di residui attraverso una trasformazione deterministica e invertibile: se il mercato fosse organizzato lungo una sola dimensione, entrambe le viste la catturerebbero e le partizioni risulterebbero concordi, mentre se discordano il mercato possiede \emph{almeno} due dimensioni indipendenti di organizzazione dei regimi e una singola etichetta è insufficiente.
L'esperimento non pretende di esaurire tutte le possibili dimensioni della struttura di mercato: dimostra che ne esistono almeno due, e fornisce un framework (il confronto cross-ARI tra partizioni indipendenti) per cercarne altre.
...
Terzo, l'asse dinamico è scoperto da un foundation model per serie temporali (MOMENT) applicato senza alcun addestramento specifico sui dati elettrici, senza fine-tuning, senza esposizione a dati di mercato durante il pre-addestramento, e senza intervento umano nella selezione delle feature.
...
Ciascuna rappresentazione viene ridotta a poche coordinate tramite Diffusion Maps~(d) e partizionata in modi di densità da ToMATo~(e).
...
Per MOMENT, un plateau ($\lambda_3$--$\lambda_5$, rapporti $\approx 1{,}04$--$1{,}08$) indica segnale residuo; il gap successivo è a $\lambda_5/\lambda_6 = 1{,}41$, dando $d = 5$.
...
\caption{Gap spettrale delle Diffusion Maps. Ogni barra rappresenta un autovalore (coordinata di diffusione), ordinato per importanza decrescente. A sinistra (FE): il salto netto dopo $\lambda_2$ indica che due coordinate bastano a catturare la struttura. A destra (MOMENT): le coordinate 3--5 formano un plateau di valori simili (tutte contribuiscono al segnale), seguito da un salto dopo $\lambda_5$ che segna il confine con il rumore. La linea rossa tratteggiata indica la dimensionalità scelta ($d = 2$ per FE, $d = 5$ per MOMENT).}
...
Qui, invece, emerge come proprietà strutturale del regime: non è la volatilità di ieri a predire quella di oggi, ma è lo stato fisico del sistema (congestioni, inerzia del parco di generazione, vincoli di rampa) a determinare simultaneamente sia la persistenza sia l'ampiezza degli shock.
