...
Applicando una metodologia interamente non supervisionata e guidata dai dati --- senza ipotesi sul numero di regimi, senza etichette fornite a priori, e senza alcun parametro fissato in anticipo --- a cinque anni di prezzi orari di ISO New England, scopriamo due assi statisticamente indipendenti di organizzazione dei regimi: è la struttura stessa dei dati a rivelare quanti regimi esistono e come si organizzano.
...
L'evidenza per la struttura bidimensionale è schiacciante sul piano statistico e rilevante sul piano economico: la sua omissione produce una distorsione sistematica e asimmetrica nell'allocazione del rischio.
...
L'elettricità viene scambiata su mercati all'ingrosso dove il prezzo può cambiare ogni ora.
A differenza del petrolio, del grano o di altre materie prime, l'elettricità non può essere immagazzinata in grandi quantità a costi ragionevoli: la capacità di batterie e pompaggio idroelettrico copre solo una piccola frazione della domanda totale.
Di conseguenza, l'offerta deve eguagliare la domanda in ogni istante, e il prezzo riflette il costo dell'ultimo generatore necessario per mantenere il sistema in equilibrio.

Questo generatore, chiamato generatore marginale, è l'unità più costosa attualmente dispacciata.
In pratica, i generatori vengono ordinati per costo di produzione (dal più economico) in una graduatoria nota come merit order.
Quando la domanda è bassa (notti, fine settimana, clima mite), i generatori di base economici (nucleare, grandi turbine a gas) sono sufficienti e i prezzi sono bassi.
Quando la domanda sale (ondate di calore, ondate di freddo) o l'offerta diminuisce (guasti ai generatori), il sistema deve ricorrere a unità progressivamente più costose --- peaker a gas e, nei casi estremi, generatori a olio combustibile --- e il prezzo sale di conseguenza.
Questo meccanismo è il motore fondamentale della formazione dei regimi \citep{weron2014}.

Negli Stati Uniti, i mercati elettrici all'ingrosso sono gestiti da Regional Transmission Organizations (RTO) e Independent System Operators (ISO).
Queste entità compensano domanda e offerta attraverso aste centralizzate che producono un Locational Marginal Price (LMP) per ogni nodo della rete di trasmissione.
Attualmente operano sette di queste organizzazioni, coprendo la maggior parte del paese; questo lavoro si concentra su ISO New England (ISONE), che serve i sei stati del New England.
La metodologia, tuttavia, non è specifica per ISONE e può essere applicata a qualsiasi mercato nodale che produca LMP orari.

La serie LMP presenta proprietà statistiche che la distinguono dalla maggior parte degli asset finanziari: code pesanti (spike estremi di prezzo), mean-reversion a scale temporali multiple, tre cicli stagionali sovrapposti --- un ciclo giornaliero (domanda giorno/notte), un ciclo settimanale (giorno feriale/fine settimana), e un ciclo annuale (domanda di riscaldamento invernale e raffrescamento estivo) --- e valori occasionalmente negativi quando la generazione rinnovabile eccede la domanda \citep{ziel2018, weron2006}.
Questi cambiamenti di regime sono guidati da una confluenza di fattori (estremi meteorologici, shock dei prezzi del combustibile, intermittenza delle rinnovabili, vincoli di trasmissione e guasti ai generatori) le cui interazioni producono la dinamica complessa e non stazionaria che rende il rilevamento dei regimi tanto importante quanto difficile.

Identificare il regime di mercato prevalente ha conseguenze pratiche dirette.
Per gli operatori di mercato, strategie di copertura regime-dipendenti possono ridurre il Value-at-Risk condizionando gli aggiustamenti di portafoglio allo stato corrente del mercato \citep{coulon2014}.
Per gli operatori di sistema e i regolatori, la permanenza prolungata in regimi ad alto prezzo segnala potenziali carenze nella struttura del mercato o la necessità di interventi di espansione della capacità.
Per i previsori, i modelli regime-switching superano costantemente i benchmark a stato singolo permettendo alla distribuzione condizionata dei prezzi di adattarsi all'ambiente operativo corrente \citep{janczura2013, nowotarski2018}.

% --- 1.2 Approcci esistenti e loro limiti ---

Il rilevamento non supervisionato dei regimi nei mercati elettrici resta una sfida metodologica aperta.
La letteratura ha affrontato il problema con tre famiglie principali di modelli, ciascuna catturando un aspetto diverso della struttura di mercato ma ciascuna con limiti significativi.

Gli \textbf{Hidden Markov Models} (HMM) \citep{mount2006, hamilton1989} identificano stati latenti e le transizioni tra essi, ma richiedono di specificare il numero di stati \emph{a priori} e assumono che le osservazioni all'interno di ciascuno stato seguano una distribuzione parametrica prefissata.
\citet{janczura2013} hanno trovato che i modelli a tre stati con varianza eterogenea forniscono il miglior fit per i prezzi Nord Pool, ma i criteri di informazione (BIC, AIC) --- gli strumenti standard per la selezione del numero di stati --- sono notoriamente inaffidabili quando il vero processo generatore si discosta dalla famiglia parametrica assunta, quasi una certezza con i prezzi elettrici a code pesanti \citep{bierbrauer2007}.

I \textbf{Gaussian Mixture Models} (GMM) \citep{kanamura2009} partizionano le osservazioni in cluster gaussiani e sono spesso usati per separare i livelli di prezzo (off-peak, normale, spike), ma dipendono anch'essi da un $K$ pre-specificato e assumono cluster convessi e simmetrici --- ipotesi difficilmente sostenibili con le distribuzioni a code pesanti tipiche dei mercati elettrici.

I \textbf{modelli mean-reverting a salti} \citep{lucia2002, cartea2005} catturano la tendenza dei prezzi a tornare verso un livello di equilibrio e sono il framework dominante per la modellazione della \emph{dinamica} dei prezzi.
Tuttavia, vincolano la velocità di mean-reversion a essere omogenea o al più modulata dal regime di prezzo: la possibilità che la velocità di mean-reversion vari indipendentemente dal livello non è contemplata.
La scelta tra un singolo parametro di mean-reversion e un parametro per regime resta una decisione di specificazione, non un risultato empirico.

Lavori più recenti hanno esplorato K-means e clustering spettrale \citep{rintamaki2017}, ma questi metodi dipendono ancora da un $K$ pre-specificato e faticano con geometrie non convesse e a densità variabile.

Questi approcci condividono due limiti strutturali.
Primo, la struttura viene imposta dall'alto: il numero di stati, la famiglia distributiva e tutti i parametri sono fissati prima di vedere i dati.
Secondo --- e più sottilmente --- ciascuna famiglia cattura implicitamente una dimensione diversa del mercato: HMM e GMM separano i \emph{livelli di prezzo}, i modelli mean-reverting descrivono la \emph{dinamica temporale}.
Ma nessun approccio si chiede se queste due dimensioni siano governate dallo stesso processo latente o da processi indipendenti --- la domanda viene semplicemente elusa, assegnando un'unica etichetta di stato che collassa livello e dinamica in una sola variabile.
Come osservato da \citet{weron2014} in una rassegna completa, l'identificazione dei regimi resta ``uno dei problemi più impegnativi del settore''.

% --- 1.3 La natura multidimensionale dei regimi ---

Un limite chiave degli approcci esistenti è che descrivono lo stato di mercato con una singola etichetta.
Tuttavia un regime di mercato ha almeno due dimensioni distinte.
La prima è il \emph{livello di prezzo}: quanto è costosa l'elettricità in questo momento --- off-peak, normale o spike?
La seconda è la \emph{dinamica dei prezzi}: come si comporta il prezzo nel tempo --- ritorna rapidamente al suo livello normale dopo uno shock, oppure lo shock persiste per giorni?
Queste due dimensioni portano informazioni diverse.
Un mercato a \$100/MWh che tornerà a \$50 entro poche ore pone un rischio diverso da un mercato a \$100/MWh che resterà elevato per due settimane, eppure un'unica etichetta di regime (``spike'') confonde i due casi.
I modelli regime-switching esistenti \citep{huisman2003} assegnano un singolo stato latente che governa simultaneamente sia il livello di prezzo sia la dinamica della volatilità, assumendo implicitamente che le due dimensioni siano guidate dallo stesso processo.
Se questa assunzione sia valida --- se livello e dinamica siano effettivamente accoppiati o varino indipendentemente --- è una domanda empirica che non è stata testata.

% --- 1.4 Nuovi strumenti ---

Due sviluppi recenti creano l'opportunità di testarla, e --- aspetto cruciale --- di farlo in modo interamente non supervisionato e guidato dai dati, eliminando ogni possibilità che scelte soggettive predeterminino il risultato.
Primo, i foundation model per serie temporali \citep{yeh2023}.
Un foundation model è una rete neurale di grandi dimensioni addestrata una sola volta su un corpus molto vasto e diversificato --- centinaia di migliaia di serie temporali da domini eterogenei (meteorologia, finanza, sanità, ingegneria) --- e poi riutilizzabile su qualsiasi nuova serie senza alcun addestramento aggiuntivo.
Questo pre-addestramento su larga scala costituisce il valore centrale dei foundation model: l'enorme diversità di pattern temporali osservati durante l'addestramento --- trend, stagionalità, persistenza, cambi di regime, code pesanti --- viene condensata nei parametri della rete, creando un patrimonio di conoscenze trasferibile a qualsiasi nuovo dominio applicativo.
Nel contesto dei rapidi sviluppi dell'intelligenza artificiale, i foundation model rappresentano uno strumento sempre più centrale per l'analisi delle serie temporali, poiché permettono di sfruttare la conoscenza accumulata su centinaia di migliaia di serie senza il costo di un addestramento dedicato.
Il modello è così in grado di produrre rappresentazioni numeriche che riassumono la struttura temporale di una finestra di input senza alcuna conoscenza specifica del dominio di applicazione.
A differenza dei modelli tradizionali che richiedono la progettazione manuale delle feature, e a differenza degli HMM e GMM che richiedono la specificazione del numero di stati, un foundation model applicato senza alcun addestramento sui dati in esame scopre autonomamente quali pattern temporali sono rilevanti.
Non è necessario decidere cosa cercare: è il modello a trovare ciò che i dati contengono.
Tra i foundation model disponibili, adottiamo MOMENT \citep{moment2024} perché soddisfa tre requisiti simultaneamente: è progettato specificamente per serie temporali (non adattato da modelli linguistici), fornisce una modalità nativa di embedding pensata per l'apprendimento delle rappresentazioni (non solo per la previsione), ed è interamente open source --- pesi, codice e dati di addestramento sono pubblicamente disponibili, garantendo la piena riproducibilità dei risultati.
Nessun altro foundation model per serie temporali (Chronos, TimesFM, Moirai) soddisfa contemporaneamente tutti e tre i requisiti.
Secondo, l'analisi topologica dei dati \citep{carlsson2009, edelsbrunner2010} offre metodi di clustering che determinano il numero di regimi direttamente dalla geometria dei dati, senza che sia necessario specificare $K$ in anticipo.
Combinati, i due strumenti realizzano una pipeline in cui nessun parametro strutturale è fissato a priori: né il numero di regimi, né la dimensionalità delle rappresentazioni, né le feature da estrarre.
Ad oggi, i foundation model per serie temporali sono stati applicati ai mercati elettrici esclusivamente per la previsione dei prezzi \citep{tsfm_epf_benchmark2025, pricefm2025} e per il rilevamento di anomalie \citep{tsfm_anomaly_energy2024} --- si veda \citet{tsfm_survey2025} per una rassegna completa --- mentre l'intero approccio qui proposto, utilizzare le rappresentazioni apprese da un foundation model come input per un clustering non supervisionato che scopra la struttura dei regimi, non ha precedenti nella letteratura sui mercati energetici.

% --- 1.4 Domanda di ricerca ---

Insieme, questi strumenti permettono di testare se livello e dinamica sono governati dalla stessa struttura latente o da processi indipendenti.
La strategia consiste nel costruire due rappresentazioni degli stessi dati di prezzo --- una che codifica le informazioni sul livello di prezzo attraverso statistiche specifiche del dominio e una che codifica le dinamiche temporali attraverso un foundation model --- e nel clusterizzare ciascuna indipendentemente con lo stesso algoritmo non parametrico.
Se le due partizioni concordano, livello e dinamica sono accoppiati e la struttura dei regimi del mercato è unidimensionale.
Se discordano, le due dimensioni variano indipendentemente, e una singola etichetta di regime è insufficiente.

% --- 1.5 Contributo ---

Questo lavoro fornisce una risposta.
Il contributo è triplice.
Primo, scopriamo un nuovo asse della struttura di mercato --- la persistenza temporale --- che è statisticamente indipendente dal livello di prezzo e invisibile a qualsiasi modello che descriva il mercato con un'unica etichetta di stato.
Secondo, la scoperta emerge da una pipeline interamente non supervisionata e guidata dai dati (Figura~\ref{fig:pipeline}): nessuna etichetta è fornita a priori, nessun numero di regimi è ipotizzato in anticipo, nessun parametro strutturale è prefissato.
È la struttura stessa dei dati a determinare quanti regimi esistono, quante dimensioni servono a descriverli e quali variabili li separano.
Terzo, l'asse dinamico è scoperto da un foundation model per serie temporali (MOMENT) applicato senza alcun addestramento specifico sui dati elettrici --- nessun fine-tuning, nessuna esposizione a dati di mercato durante il pre-addestramento, nessun intervento umano nella selezione delle feature.
Il modello riceve esclusivamente i residui destagionalizzati $r_t$, senza accesso al livello di prezzo: qualsiasi struttura catturi proviene puramente dalla dinamica temporale della serie.
Questa tripla indipendenza --- dal dominio, da ipotesi soggettive e dalla supervisione --- rende il risultato particolarmente robusto: non viene deciso a priori cosa cercare, ma sono i dati stessi a rivelare che la persistenza è la dimensione organizzativa dominante non catturata dalle statistiche distribuzionali.

La pipeline (Figura~\ref{fig:pipeline}) prima preprocessa la serie oraria dei prezzi~(a), poi costruisce due rappresentazioni delle stesse finestre scorrevoli --- feature costruite manualmente~(b) e un embedding da foundation model~(c).
Ciascuna rappresentazione viene ridotta a un piccolo numero di coordinate tramite Diffusion Maps~(d) e partizionata in modi di densità da ToMATo~(e).
Una diagnostica di separazione delle feature~(f) valuta tutte le 15 feature interpretabili rispetto a entrambi gli insiemi di modi per scoprire cosa separa ciascuna rappresentazione, e la variabile di merge risultante viene utilizzata per consolidare i modi tramite Tukey HSD~(g).
Applicando questa pipeline a cinque anni di LMP orari di ISO New England, troviamo che le due partizioni risultanti sono statisticamente indipendenti: il mercato elettrico è organizzato lungo almeno due assi ortogonali di struttura dei regimi, uno economico e uno dinamico.
...
\node[box, fill=yellow!10, draw=red!50, dashed, thick, below=of regimes]
  (axes) {\textcolor{red!65!black}{\textbf{Due assi indipendenti}}};
...
L'ACF al lag $h$, indicata con $\rho(h)$, misura la correlazione lineare tra $y_t$ e $y_{t+h}$: un valore vicino a 1 indica che la serie si ripete $h$ passi dopo, mentre un valore vicino a 0 indica assenza di tale ripetizione.
...
\item I suoi \textbf{incrementi} $\Delta r_t = r_t - r_{t-1}$: descrivono la distribuzione delle innovazioni --- quanto sono ampi gli shock, se sono simmetrici, se hanno code pesanti. La serie $\Delta r_t$ è stazionaria (ACF $\approx -0{,}08$).
\end{itemize}
...
L'\textbf{encoder temporale} (MOMENT) è un modello di sequenza che processa i valori nel loro ordine: la persistenza del processo è il segnale che deve codificare, non un confondente.
...
Il passo $S = 6$ ore è scelto in modo che i tempi di soggiorno nei regimi ($\sim$60\,h, Sezione~\ref{sec:economic_axis}) siano risolti da circa 10 osservazioni per soggiorno.
...
La pipeline (Figura~\ref{fig:pipeline}) è completamente non supervisionata: il numero di regimi e tutti i parametri emergono dai dati.
...
Come discusso nella Sezione~\ref{sec:two_views}, ciascun strumento riceve la rappresentazione del residuo più coerente con la propria struttura computazionale:
\begin{equation}
...
\end{equation}
FE riceve gli incrementi $\Delta r_i$ (stazionari) e la finestra di prezzi grezzi $p_i$; le sue statistiche order-invariant misurano così la distribuzione delle innovazioni senza contaminazione dalla persistenza.
MOMENT riceve i livelli $r_i$ (persistenti): il suo encoder di sequenza codifica la struttura di dipendenza temporale --- la persistenza è il segnale, non un confondente.

Le due rappresentazioni derivano dalla stessa serie (Sezione~\ref{sec:two_views}), ma ciascun strumento ne vede la forma più adatta al proprio funzionamento.
MOMENT non ha accesso ai livelli di prezzo assoluti: qualsiasi struttura catturi proviene dalla dinamica temporale, non dal livello di prezzo.

Dopo il clustering, ogni rappresentazione assegna ogni finestra a un gruppo --- questa assegnazione si chiama partizione.
L'esperimento è progettato come un confronto: entrambe le rappresentazioni vengono processate attraverso la stessa pipeline, e le due partizioni risultanti vengono confrontate alla fine.
Se le partizioni concordano, un singolo asse di regime è sufficiente a descrivere il mercato.
Se discordano, il mercato richiede più di un asse.
...
Si noti che FE non include feature di persistenza temporale (ACF): essendo calcolate sugli incrementi $\Delta r_t$ (stazionari, ACF~$\approx 0$), le autocorrelazioni sarebbero prossime a zero e non informative. La persistenza --- misurata dall'ACF sui livelli $r_t$ --- è lasciata interamente alla seconda rappresentazione.
...
Per FE, il gap è netto tra $\lambda_2$ e $\lambda_3$ (rapporto 2,26), dando $d = 2$.
Per MOMENT, un plateau ($\lambda_3$--$\lambda_5$, rapporto $\approx 1{,}05$) indica segnale residuo; il gap successivo è a $\lambda_5/\lambda_6 = 1{,}41$, dando $d = 5$.
...
Per ciascuna delle 19 feature diagnostiche (15 FE + 4 ACF calcolate sui livelli $r_t$), calcoliamo il rapporto di correlazione $\eta^2$ \citep{fisher1925} rispetto a entrambe le partizioni.
$\eta^2$ misura la quota di varianza della feature spiegata dai cluster: valori alti indicano che il clustering separa bene quella feature.
...
\toprule
Feature & $\eta^2_\text{FE}$ & $\eta^2_\text{MOM}$ & Feature & $\eta^2_\text{FE}$ & $\eta^2_\text{MOM}$ \\
\midrule
LMP medio   & \textbf{0,495} & 0,085 & ACF 6h     & 0,181 & \textbf{0,420} \\
LMP $Q_{95}$ & \textbf{0,402} & 0,145 & ACF 1h     & 0,161 & \textbf{0,376} \\
IQR         & 0,336 & 0,012 & ACF 24h    & 0,114 & \textbf{0,346} \\
LMP std     & 0,311 & 0,165 & ACF 168h   & 0,007 & 0,030 \\
vol\_24h    & 0,303 & 0,015 & Curtosi    & 0,295 & 0,049 \\
Std $\Delta r$ & 0,293 & 0,020 & Range   & 0,235 & 0,045 \\
$Q_5$       & 0,294 & 0,016 & Max        & 0,181 & 0,048 \\
$Q_{95}$    & 0,267 & 0,020 & Asimmetria & 0,154 & 0,016 \\
Min         & 0,271 & 0,031 & Mediana    & 0,041 & 0,003 \\
Media       & 0,039 & 0,014 &            &       &       \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Merge Tukey HSD sull'asse economico. In alto: 48 modi iniziali ToMATo, rappresentati per LMP medio. Molti modi hanno prezzi quasi identici. In basso: dopo il merge iterativo, sopravvivono 9 regimi, ciascuno significativamente diverso da tutti gli altri.}
...
Primo, i due regimi dominanti --- $D_0$ (38,6\%) e $D_2$ (33,3\%) --- coprono insieme il 72\% delle finestre. Il mercato trascorre la maggior parte del tempo in condizioni di persistenza moderata-alta (ACF 0,68--0,75), con escursioni meno frequenti verso regimi a rapida reversione ($D_7$, ACF $= 0{,}38$) o shock molto persistenti ($D_8$, ACF $= 0{,}94$).
...
Questo accoppiamento persistenza--volatilità è coerente con il volatility clustering nei mercati finanziari, ma qui viene catturato come proprietà strutturale del regime piuttosto che come effetto autoregressivo.
Questo è atteso --- la persistenza temporale può variare più rapidamente del livello di prezzo sottostante --- ma i regimi sono comunque temporalmente coerenti alla scala sub-giornaliera, non rumore casuale.
...
% --- ARI ---
Per rispondere a questa domanda, serve un singolo numero che riassuma quanto le due partizioni si sovrappongono.
Utilizziamo l'Adjusted Rand Index (ARI) \citep{hubert1985}.
L'idea è diretta: se due finestre finiscono nello stesso regime economico, tendono anche a finire nello stesso regime dinamico?
Se sì, le due partizioni sono ridondanti --- dividono i dati in modi simili.
Se no, sono indipendenti --- ciascuna raggruppa le finestre secondo un proprio criterio, ignorando ciò che fa l'altra.
L'ARI cattura questo come un numero tra 0 e 1, corretto in modo che la concordanza casuale valga 0 e la concordanza perfetta valga 1.
Formalmente, sia $n_{ij}$ il numero di finestre assegnate al regime economico $i$ e al regime dinamico $j$, e siano $a_i = \sum_j n_{ij}$ e $b_j = \sum_i n_{ij}$ i totali di riga e colonna.
L'ARI è
\begin{equation}
\text{ARI} = \frac{\sum_{ij}\binom{n_{ij}}{2} - \left[\sum_i\binom{a_i}{2}\sum_j\binom{b_j}{2}\right]\!/\!\binom{n}{2}}{\frac{1}{2}\!\left[\sum_i\binom{a_i}{2}+\sum_j\binom{b_j}{2}\right] - \left[\sum_i\binom{a_i}{2}\sum_j\binom{b_j}{2}\right]\!/\!\binom{n}{2}}.
\label{eq:ari}
\end{equation}
Il numeratore conta le concordanze tra coppie oltre il caso; il denominatore normalizza per il massimo eccesso possibile.

L'ARI tra le partizioni economica e dinamica è \textbf{0,012}.

Questo valore è molto vicino a zero --- ben al di sotto della soglia di 0,5 tipicamente considerata indicativa di concordanza significativa.
In termini pratici, significa che conoscere il regime economico di una finestra non ci dice quasi nulla sul suo regime dinamico, e viceversa.
...
Il valore dell'ARI è la prima di tre linee di evidenza indipendenti a supporto di questa quasi-ortogonalità.

% --- Evidenza 2: separazione visiva ---
La seconda è visiva.
...
% --- Evidenza 3: quattro quadranti ---

L'indipendenza statistica dei due assi solleva una domanda immediata: questa struttura bidimensionale ha conseguenze pratiche, o è solo un'osservazione statistica?
...
\caption{Traiettorie empiriche (sopra, blu) e Monte Carlo simulate (sotto, rosso) per tre regimi dinamici all'interno di $E_3$ (Baseload, \$41/MWh). A sinistra: rapida reversione ($\alpha = 0{,}159$, half-life 4\,h). Al centro: moderata ($\alpha = 0{,}060$, half-life 11\,h). A destra: persistente ($\alpha = 0{,}031$, half-life 22\,h). L'area ombreggiata indica l'inviluppo teorico $\pm 2\sigma_\infty$. Tutte le finestre provengono dallo stesso regime di prezzo ma mostrano dinamiche qualitativamente diverse.}
...
Dal punto di vista della microstruttura, l'asse $D$ mappa l'elasticità temporale dell'offerta e i vincoli fisici della rete --- congestioni, rampe di accensione dei generatori termici, inerzia del parco di generazione --- che sono per loro natura ortogonali al livello nominale del prezzo.
...
L'asse dinamico, scoperto dalla pipeline senza alcuna assunzione parametrica, fornisce la dimensione mancante e consente una caratterizzazione del rischio che riflette i vincoli fisici del mercato, non solo il suo livello nominale di prezzo.
...
Tutte e quattro le combinazioni di prezzo alto/basso e mean-reversion rapida/lenta sono popolate (Figura~\ref{fig:two_axes}), ciascuna con un significato operativo distinto.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.58\textwidth]{two_axis_grid.png}
\caption{Spazio dei regimi a due assi. Ogni quadrante rappresenta uno stato di mercato distinto. Tutti e quattro i quadranti sono popolati, confermando che livello di prezzo e persistenza variano indipendentemente.}
\label{fig:two_axes}
\end{figure}
...
Questo lavoro introduce una pipeline non parametrica per il rilevamento non supervisionato dei regimi nei mercati elettrici spot, in cui il numero di regimi, la dimensionalità della varietà e tutti i parametri di regolazione emergono dai dati attraverso l'analisi della persistenza topologica.
Applicando questa pipeline a cinque anni di prezzi orari di ISO New England con due rappresentazioni alternative, arriviamo a una scoperta strutturale: il mercato elettrico è organizzato lungo almeno due assi indipendenti di struttura dei regimi.

Il primo asse è economico: 15 feature --- 12 distribuzionali sugli incrementi $\Delta r_t$ e 3 statistiche sull'LMP grezzo --- producono $K_E = 9$ regimi che vanno dall'off-peak allo spike estremo invernale.
...
I due assi sono statisticamente indipendenti.
